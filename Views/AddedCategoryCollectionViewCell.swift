//
//  AddedCategoryCollectionViewCell.swift
//  ExpenseTracker
//
...
import UIKit
class AddedCategoryCollectionViewCell: UICollectionViewCell {
    static let identifier = "AddedCategoryCollectionViewCell"
    ...
    func configure(categoryTotalAmount: Double, categoryName: String, categories: [ExpenseCategory]) {
        let totalAmountString = "₦" + String(format: "%.2f", categoryTotalAmount)
        amountLabel.text = totalAmountString

        nameLabel.text = categoryName

        // Find the corresponding category in the passed categories array
        if let category = categories.first(where: { $0.name == categoryName }) {
            // Set the image of the iconCategoryImageView with the category's icon
            iconCategoryImageView.image = UIImage(systemName: category.icon!)
        }
