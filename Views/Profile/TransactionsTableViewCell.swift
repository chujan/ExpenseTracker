...
class TransactionsTableViewCell: UITableViewCell, MFMessageComposeViewControllerDelegate {
    ...
    private func presentSMSComposer() {
           if MFMessageComposeViewController.canSendText() {
               ...
               messageVC.body = "This is a test SMS notification"
               messageVC.messageComposeDelegate = self
               // Find a way to present this VC. This might need to be done by the parent view controller
           } else {
    ...
    @objc private func smsSwitchValueChanged(_ sender: UISwitch) {
            if sender.isOn {
                presentSMSComposer()
                UserDefaults.standard.set(true, forKey: "smsNotificationsEnabled")
            } else {
                // Handle disabling SMS notifications if needed
                UserDefaults.standard.set(false, forKey: "smsNotificationsEnabled")
            }
        }
    
    func messageComposeViewController(_ controller: MFMessageComposeViewController, didFinishWith result: MessageComposeResult) {
            // Handle the result of the message compose view controller
